...
if (!class_exists('WP_Advanced_Search')) {
    class WP_Advanced_Search {
                ...
                foreach ($fields as $field) {
                    if (isset($field['type'])) {
                        switch($field['type']) {
                            ...
                            case 'reset':
                                if (!$has_reset) {
                                    $output .= $this->reset_button($field);
                                    $has_reset = true;
                                }
                                break;    
                        } //end switch
         ...
         * @since 1.0
         */
        function reset_button( $args ) {
            $defaults = array('value' => 'Reset');
            $args = wp_parse_args($args, $defaults);
            extract($args);
            $args['values'] = $value;
            $args['format'] = 'reset';
            $field = new WPAS_Field('reset', $args);
            return $field->build_field();
        }
        ...
        function build_meta_query() {
            ...
            foreach ($meta_keys as $key => $values) {
                ...
                if ($this->meta_keys[$key]['compare'] == 'BETWEEN') {
                    ...
                    $customKey = $key;
                    if('speaker_date_from' == $customKey || 'speaker_date_to' == $customKey){
	                    $customKey = 'speaker_date';
	                    $values = date('Ymd', strtotime($values));
                    }
                    $this->wp_query_args['meta_query'][] = array(   
                                                            'key' => $customKey,
                                                            'value' => $values,
