...
Class WPAS_Field {
	...
	function date() {
        ...
        if ($this->placeholder)
            $placeholder = ' placeholder="'.$this->placeholder.'"';
        $output = '<input type="date" id="'.$this->id.'" value="'.$value.'" name="'.$this->id.'"'.$placeholder.'>';
        if ($this->sublabel) {
	        $output .= '<br/><span class="sublabel">'.$this->sublabel.'</span>';
        }
        return $output;
    }
    function textarea() {
	...
	function reset() {
        $output = '<input type="reset" value="'.esc_attr($this->values).'">';
        return $output;
    }
